    ...
    public function actAdd(): Json
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                if($this->userInfo['grade_id'] > $this->area_grade) {
                    if(!$this->request['region_ids']){
                        throw new \Exception('区域id不能为空');
                    }
                    $region_ids = $this->request['region_ids'];
                }else{
                ...
                $school_attr_array = explode(',',$data['school_attrs']);
               ...
               foreach($planInfo as $key=>$value){
                ...
                foreach($school_attr_array as $key=>$value){
                    if(array_key_exists($value,$school_attr_tmp)){
                        $school_attr_text .= $school_attr_tmp[$value]."/";
                    }
                }
                ...
                if(is_array($user_ids)){
                    foreach($user_ids as $key=>$value){
                        foreach(explode(',',$region_ids) as $_key => $_value){
                            ...
                            $add_data['school_attr'] = $data['school_attrs'];
