...
class PrimarySchoolRoll extends Education
{
    /**
     * 按分页获取学籍信息
            ...
            try {
                ...
                if($role_school['school_ids'] ){
                    $school_ids = Db::name("SysSchool")
                        ->where('id', 'in', $role_school['school_ids'])->where('school_type', 1)
    ...
    public function actDelete()
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                //学籍统计
                $school_id = Db::name('SysSchoolRoll')->where('id', $data['id'])->value('school_id');
...
//                            $error[] = '第' . $row . '行学校标识码为【' . $item['school_code'] . '】的学校类型为初中';
//                            continue;
//                        }
                    ...
                    Cache::set('importError_primary_school_roll_'.$this->userInfo['manage_id'], $error);
    ...
    public function getImportError(): Json
    {
        if ($this->request->isPost()) {
            try {
                $errData = Cache::get('importError_primary_school_roll_'.$this->userInfo['manage_id']);
                if(empty($errData)){
                    $errData = [];
                }
                $data['data'] = $errData;
...
//                if($role_school['bounded']  ){
//                    $where[] = ['roll.school_id', 'in', $role_school['school_ids']];
//                }
    ...
    private function getRecruit(): array
    {
        try{
            ...
            if ($this->userInfo['grade_id'] >= $this->city_grade) {
                ...
                //小学学校ID
                $school_ids = Db::name("SysSchool")
                    ->where('school_type', 1)->where('id', 'in', $role_school['school_ids'])
                    ->where('deleted', 0)->where('disabled', 0)->column('id');
                //批复学位
