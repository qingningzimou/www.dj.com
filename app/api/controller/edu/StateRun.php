...
class StateRun extends Education
{
    /***********************************************************************************/
    //--------------------录取页面-------------------------------
    /***********************************************************************************/
    ...
    public function getResultList(): Json
    {
        if ($this->request->isPost()) {
            try {
                $result = $this->getListByType(4, true);
                if($result['code'] == 0){
    ...
    public function resView(): Json
    {
        if ($this->request->isPost()) {
            try {
                ...
                $result['data']['admission_type_list'][] = ['id' => 8, 'name' => '摸排'];
    ...
    public function actInit(): Json
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                $where[] = ['deleted', '=', 0];
                $where[] = ['voided', '=', 0];
                $where[] = ['admission_type', 'not in', [2, 3] ];
                $paid_count = Db::name('UserApply')->where($where)->count();
                if($paid_count > 0 ){
                    throw new \Exception('选择的学生不是线上审核或到校审核录取！');
                }
                ...
                $where[] = ['deleted', '=', 0];
                $where[] = ['voided', '=', 0];
                $where[] = ['admission_type', 'in', [2, 3] ];//没有民办缴费
    ...
    public function exportResult(): Json
    {
        ...
        {
            try {
                $result = $this->getListByType(4, false);
                if($result['code'] == 0){
                    throw new \Exception($result['msg']);
                }
                $data = $result['list'];
                ...
                if(count($data) > 10000){
                    ...
                    for ($i = 0; $i < $count_excel; $i++){
                        ...
                        $this->excelExport('公办学校最终录取_' . ($i + 1) . '_', $headArr, $data);
                    }
                }else {
                    $this->excelExport('公办学校最终录取_', $headArr, $data);
                }
