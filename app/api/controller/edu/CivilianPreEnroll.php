...
class CivilianPreEnroll extends Education
{
    /**
     * 预录取列表
     * @return Json
    ...
    public function actAdmission(): Json
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                /*$admission_count = $result['data']['prepared_count'];
    ...
    public function exportPrepare(): Json
    {
        ...
        {
            try {
                $result = $this->getListByType(2, false);
                if($result['code'] == 0){
                    throw new \Exception($result['msg']);
                }
                $data = $result['list'];
                ...
                if(count($data) > 10000){
                    ...
                    for ($i = 0; $i < $count_excel; $i++){
                        ...
                        $this->excelExport('民办学校预录取_' . ($i + 1) . '_', $headArr, $data);
                    }
                }else {
                    $this->excelExport('民办学校预录取_', $headArr, $data);
                }
    ...
    private function getListByType($type, $is_limit = true): array
    {
        try {
            ...
            if ($type == 2) {
                ...
                $where[] = ['a.paid', '=', 0];//未缴费
                $where[] = ['a.signed', '=', 0];//没有入学报到
                $where[] = ['a.result_school_id', '=', $school_id];//最终录取学校ID
            }
            //录取
            if ($type == 3) {
                $where[] = ['a.prepared', '=', 1];//预录取
