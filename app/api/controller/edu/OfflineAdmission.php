...
class OfflineAdmission extends Education
{
    ...
    public function getSelectList(): Json
    {
        if ($this->request->isPost()) {
            ...
            foreach ($houseType['data'] as $datum){
                $result['house_list'][] = [
                    'id' => $datum['dictionary_value'],
                    'name' => $datum['dictionary_name'],
                ];
            }
    ...
    public function actAdd()
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                $school = Db::name('SysSchool')->find($school_id);
                if(!$school){
                    throw new \Exception('管理员关联学校ID信息不存在');
                }
                if($school['school_attr'] == 2){
                    throw new \Exception('学校的性质【民办】');
                }
                if($school['onlined'] == 1){
                    throw new \Exception('学校的招生方式为【线上】');
                }
                ...
                $child_id = Db::name("UserChild")->where('idcard', $id_card)->where('deleted', 0)->value('id');
                if(!$child_id){
                ...
                $guardian_id_card = strtoupper($data['guardian_id_card']);
                $family_id = Db::name("UserFamily")->where('idcard', $guardian_id_card)->where('deleted', 0)->value('id');
                if(!$family_id) {
                    $family = [];
                ...
                $house_id = Db::name('UserHouse')->where('family_id', $family_id)->where('deleted', 0)->value('id');
                if(!$house_id) {
                    $house = [];
                ...
                $applyInfo = Db::name('UserApply')->where($where)->find();

                if($applyInfo){
                    if($applyInfo['prepared'] == 1 && $applyInfo['resulted'] == 1 ){
                        if($applyInfo['region_id'] == $school['region_id']) {
                            throw new \Exception('学生已在本区录取');
                        }else{
                            throw new \Exception('学生已在其他区域录取');
                        }
                    }
                    if($applyInfo['prepared'] == 1 && $applyInfo['resulted'] == 0){
                        if($applyInfo['region_id'] == $school['region_id']) {
                            throw new \Exception('学生已在本区预录取');
                        }else{
                            throw new \Exception('学生已在其他区域预录取');
                        }
                    }
                    $apply = [
                        'id' => $applyInfo['id'],
                    ...
                    $result = (new model())->editData($apply);
                    if($result['code'] == 0){
                        throw new \Exception($result['msg']);
                    }
                    $user_apply_id = $applyInfo['id'];
                }else{
                    $apply = [
                        'enrol_year' => date('Y'),
                        'region_id' => $school['region_id'],
                    ...
                    $result = (new model())->addData($apply, 1);
                    if($result['code'] == 0){
                        throw new \Exception($result['msg']);
                    }
                ...
                if($applyDetail){
                    ...
                    $detail['child_name'] = $data['real_name'];
                    $detail['child_idcard'] = $id_card;
                    $detail['school_attr'] = $school['school_attr'];
                    $detail['school_type'] = $school['school_type'];
                    $detail['house_type'] = $data['house_type'];
    ...
    public function actEdit()
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                $child_id = Db::name("UserChild")->where('idcard', $id_card)->where('deleted', 0)->value('id');
                if($apply['child_id'] != $child_id){
                    throw new \Exception('学生身份证号不能修改');
                }
                ...
                if($applyDetail){
                    $detail = [];
                    $detail['id'] = $applyDetail['id'];
                    $detail['user_apply_id'] = $apply['id'];
                    $detail['child_id'] = $apply['child_id'];
                    $detail['mobile'] = $data['mobile'];
                    $detail['child_name'] = $data['real_name'];
                    $detail['child_idcard'] = strtoupper($data['student_id_card']);
                    $detail['school_attr'] = $apply['school_attr'];
                    $detail['school_type'] = $apply['school_type'];
                    $detail['house_type'] = $data['house_type'];
...
//                            }
                    ...
                    Cache::set('importError_offlineAdmission_'.$this->userInfo['manage_id'], $error);
    ...
    public function getImportError(): Json
    {
        if ($this->request->isPost()) {
            try {
                $res = [
                    'code' => 1,
                    'data' => Cache::get('importError_offlineAdmission_'.$this->userInfo['manage_id']),
                ];
            } catch (\Exception $exception) {
                $res = [
                    'code' => 0,
    ...
    public function export()
    {
        if($this->request->isPost())
        {
            try {
                ...
                $headArr = ['编号', '学生姓名', '学生身份证号', '监护人姓名', '监护人身份证号', '联系手机号', '房产类型', '房产所有人', '房产地址', '毕业学校'];
                if(count($data) > 100000){
                    ...
                    for ($i = 0; $i < $count_excel; $i++){
                        ...
                        $this->excelExport('学生录取名单_' . ($i + 1) . '_', $headArr, $data);
                    }
                }else {
                    $this->excelExport('学生录取名单_', $headArr, $data);
                }
