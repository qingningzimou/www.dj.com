...
use app\common\validate\account\AreaManage as validate;
use think\App;
 ...
 * */
class AreaManage extends Education
{
    private $role_id;
    private $main;
    public function __construct(App $app)
    {
        ...
        $getData = $dictionary->findValue('dictionary', 'SYSJSBM', 'SYSQJBM');
        $this->role_id = $getData['data'];
        $this->main = 0;
        if($this->area_grade < $this->userInfo['grade_id'] || $this->userInfo['main_account']){
            $this->main = 1;
        }
    }
    ...
    public function getList()
    {
        if ($this->request->isPost()) {
            try {
                ...
                $data = $this->getManageList($where,$this->area_grade);
    ...
    public function actAdd()
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                //验证管理员权限
                if($this->area_grade >= $this->userInfo['grade_id']  && !$this->userInfo['defaulted']){
                ...
                $manageData['role_grade'] = $this->area_grade;
    ...
    public function actDelete()
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                if ((!$chkeck_region || $this->area_grade >= $this->userInfo['grade_id']) &&  !$this->userInfo['defaulted']) {
                    throw new \Exception('权限不足以进行此操作');
                }
    ...
    public function resView()
    {
        if ($this->request->isPost()) {
            ...
            $data['department'] = filter_by_value($department, 'grade_id', $this->area_grade);
