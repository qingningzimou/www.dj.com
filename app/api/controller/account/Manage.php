            ...
            try {
                ...
                if (!empty($user_info)) {
                    ...
                    if(!$user_info['user_type']){
                        Db::name('user')->where('id',$user_id)->update(['user_type' => 1,'password' => $password,'refresh_pass' => 1]);
                    }else{
                        Db::name('user')->where('id',$user_id)->update(['password' => $password,'refresh_pass' => 1]);
                    }
                }else{
                ...
                $roleNodes = (new SysRoleNodes())->where('role_id',$postData['role_id'])->whereIn('node_id',$chk_node_ids)->field(['node_id','node_type'])->select()->toArray();
                $saveData = [];
        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                $chk_node_ids = (new SysNodes())
                    ...
                    ->column('id');
                $roleNodes = (new SysRoleNodes())->where('role_id',$data['role_id'])->whereIn('node_id',$chk_node_ids)->field(['node_id','node_type'])->select()->toArray();
                ...
                Db::name('manage')
                    ->where('id','<>',$data['id'])
                    ->where('user_id',$manage_info['user_id'])
                    ->update([
                        'user_name' => $data['user_name'],
                        'real_name' => $data['real_name'],
                        'simple_code' => $data['simple_code']
    ...
    public function resetPassword()
    {
        if ($this->request->isPost()) {
            try {
                ...
                $sms = new SmsSend();
                $msg = "$password 是您在襄阳市招生平台的新密码，请尽快登录修改。【顽磁科技】";
                Cache::delete('user'.md5($manage_info['user_id']));
                $res = $sms->send_sms($manage_info['user_name'],$msg);
            } catch (\Exception $exception) {
                $res = [
                    'code' => 0,
                    'msg' => $exception->getMessage() ?: Lang::get('system_error')
