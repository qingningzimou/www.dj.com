...
class Business extends Education
{
    /**
     * 业务办理列表信息
...
//                    $where[] = ['a.result_school_id', 'not in', $school_ids ];
                ...
                {
                    $where[] = ['d.house_type', 'in', explode(',', $this->result['house_type']) ];
                }
                //三证情况
                if($this->request->has('three_syndromes_status') && $this->result['three_syndromes_status'] !== '')
                {
                    ...
                    //$where[] = ['d.three_syndromes_status','=', $this->result['three_syndromes_status']];
                }
                //学籍情况
...
//                $where[] = ['voided', '=', 0];
//                $where[] = ['paid', '=', 1];
//                $where[] = ['admission_type', 'not in', [4, 6] ];
//                //$where[] = Db::Raw("(paid = 1 and  )");
//                $paid_count = Db::name('UserApply')->where($where)->count();
//                if($paid_count > 0 ){
//                    throw new \Exception('选择的学生已民办缴费！');
//                }
                ...
                $update['public_school_id'] = 0;//公办申请学校ID
...
//                        }
                        ...
                        if(!$region_id){
                            $error[] = '第' . $row . '行申请区县填写错误';
                            continue;
                        }
                        if(!in_array($region_id, $region_ids)){
                            $error[] = '第' . $row . '行申请区县【' . $item['region_name'] . '】无权限管理';
                            continue;
                        }
                        ...
                        if($item['result_school_name'] != ''){
                            ...
                            $region_id = $school['region_id'];
                            $auto_admission = $school['auto_admission'];
                        }
                        ...
                        if($school_attr == 2){
                            if($auto_admission) {
                                ...
                                //民办报名信息列表
                                $apply_data[$key]['prepared'] = 0;
                                $apply_data[$key]['resulted'] = 0;
                                $apply_data[$key]['apply_status'] = 1;//未录取
                                $apply_data[$key]['apply_school_id'] = $school_id;
                            }
                        }
...
//                        $successNum++;
                    ...
                    foreach ($apply_data as $k => $v){
                        if($v['school_id'] == 0 || ($v['school_attr'] == 1 && $v['public_school_id'] > 0 ) ||
                            ($v['apply_school_id'] > 0 && $v['school_attr'] == 2 ) ) {
